...
with st.sidebar:
    ...
    # Initialize defaults before the if/else so they always exist
    # (avoids NameError if an exception occurs during file loading)
    ...
    if uploaded_file:
        try:
            ...
            if 'Chauffeur' in df.columns:
                ...
                drivers = [all_opt] + sorted(df['Chauffeur'].dropna().astype(str).unique().tolist())
